<template>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb my-0 px-2 py-1">
            <li class="breadcrumb-item breadcrumb-item-slash" :class="{'active': p.last}" v-for="p in parsedList" :key="p.id">
        ...
        setup(props) {
            const state = reactive({
                parsedList: computed(_ => {
                    let l = [];
                    const revIds = props.entity.parentIds.slice().reverse();
                    const revNames = props.entity.parentNames.slice().reverse();
