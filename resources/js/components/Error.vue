    ...
    <modal name="error-modal" height="80%" @before-open="receiveErrorMessage">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close" aria-label="Close" @click="hideErrorModal">
                </button>
            </div>
            <div class="modal-body col d-flex flex-column">
                <p v-if="hasRequest">
                    <code>{{ request.method }}</code> Request <a :href="request.url">{{ request.url }}</a> failed with status <span class="fw-medium">{{ request.status }}</span>.
                </p>
                <p class="alert alert-danger col scroll-y-auto">
                    Error Message: <span class="fw-light font-italic">{{ msg.error || msg }}</span>
                </p>
