...
class EntityController extends Controller {
    ...
    function exportEntityTree($id, Request $request) {
        ...
        while($parentEntity = array_shift($parents)) {

            // Add entity to array
            $data = [];
            $data['name']=$parentEntity->name;
            $data['parent']=implode("\\", $parentEntity->getAncestorsAttribute());

            $data['entity_type']= $parentEntity->entity_type->thesaurus_concept->getActiveLocaleLabel();
     

            $entities[] = $data;

            // Get all children and add them to the Queue
            $child_entities = Entity::getEntitiesByParent($parentEntity->id);
