...
class ExportEntityTypes extends Command {
    ...
    protected function printerLogFileName(): string {
        return "transfer.log";
    }
    ...
    public function handle(): int {
        ...
        try {
            ...
            if($this->confirm('Check the receipt carefully before proceeding')) {
                ...
                // This path did not return anything. I return 0 here (see bash return codes)
                return 0;
            } else {
    ...
    private function extractThesaurusConcepts($model, array &$allConcepts): void {
    ...
    private function collectAllDataFromEntityChildren(Entity $entity): array {
        $thesaurusLabels = [];
    ...
    private function collectLanguagesForConcept($concept, &$thesaurusLabels): void {
    ...
    private function collectEntityRelations(array $entityTypes): array {
        $entityTypesMap = [];
        foreach($entityTypes as $entityType) {
    ...
    private function transferData($user, $data): void {
    ...
    private function printReceipt($data): void {
    ...
    private function transferThConcepts(User $user, array $data): void {
        foreach($data as $concept){
            $result = ThConcept::on(static::$destination)
                ->where('concept_url', $concept->concept_url)
                ->first();
    ...
    private function transferThLabels(User $user, array $thLabels): void {
        foreach($thLabels as $label) {
            $newConceptId = $this->thConceptOldIdToIdMap[$label->concept_id];
            $result = ThConceptLabel::on(static::$destination)
                ->where('concept_id', $newConceptId)
                ->where('language_id', $label->language_id)
                ->first();
    ...
    private function transferEntityTypes(User $user, array $entityTypes): void {
        foreach($entityTypes as $entityType) {
            $result = EntityType::on(static::$destination)
                ->where('thesaurus_url', $entityType->thesaurus_url)
                ->first();
    ...
    private function transferAttributes(User $user, array $attributes): void {
        foreach($attributes as $attribute) {
            $result = Attribute::on(static::$destination)
                ->where('thesaurus_url', $attribute->thesaurus_url)
                ->where('datatype', $attribute->datatype)
                ->first();
    ...
    private function transferEntityTypeAttributeRelations(User $user, array $entityTypeAttributes): void {
        foreach($entityTypeAttributes as $entityTypeAttribute) {
            ...
            $result = EntityAttribute::on(static::$destination)
                ...
                ->where('metadata', $entityTypeAttribute->metadata)
                ->first();
    ...
    private function transferBroaderNarrowerRelations(): void {
        foreach($this->narrowerBroaderRelation as $relation) {
            ...
            $result = ThBroader::on(static::$destination)
    ...
    private function transferEntityTypeRelations(array $entityTypeRelations): void {
    ...
    private function ellipsis($string, $max = 50): string {
    ...
    private function exactOrEllipsis($string, $max = 50): string {
        if(strlen($string) > $max) {
